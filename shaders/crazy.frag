...
vec4 negate(vec4 col)
{
	float dx = mousePos.x - gl_FragCoord.x;
	float dy = mousePos.y - gl_FragCoord.y;
	
	if (sqrt(dx*dx + dy*dy) < negDist)
	{
		if (Normal.z > 0.0) discard;
		return texture(lensTex, TexCoord);
	}
